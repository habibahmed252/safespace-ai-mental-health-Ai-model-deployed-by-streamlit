...
st.markdown(f'<div dir="{direction}" class="hero"><h1>SafeSpace AI</h1><div class="hero-sub">{esc(T["subtitle"])}</div><div class="creator-name">{esc(T["creator"])}</div></div>', unsafe_allow_html=True)
st.markdown(f'<div dir="{direction}" class="card"><div class="about-title">{esc(T["about"])}</div><div class="about-text">{esc(T["about_desc"])}</div></div>', unsafe_allow_html=True)
...
with st.expander(T["model_section"], expanded=False):
    ...
    st.markdown(f'<div dir="{direction}" class="card" style="margin:0;padding:16px">{model_text}</div>',unsafe_allow_html=True)
...
with right:
    st.markdown(f'<div dir="{direction}" class="ai-card" style="min-height:230px"><div class="chat-icon">💬</div><div class="ai-title">{esc(T["response"])}</div><div class="ai-copy">{esc(T["response_wait"])}</div></div>',unsafe_allow_html=True)
...
if results:
    ...
    with r2:
        ...
        if support:
            st.markdown(f'<div dir="{direction}" class="ai-card"><div class="chat-icon">💬</div><div class="ai-title">{esc(T["response"])}</div><div class="ai-copy">{esc(support)}</div></div>',unsafe_allow_html=True)
        else:
        ...
        st.markdown(f'<div dir="{direction}" class="guidance-card"><div class="ai-title">{esc(T["guidance"])}</div><div class="ai-copy">{esc(consolation)}</div><div class="ai-title" style="margin-top:12px">🛡️ {esc(T["selfcare"])}</div><ul>{steps_html}</ul></div>',unsafe_allow_html=True)
        if suicidal_prob >= 0.25:
            st.markdown(f'<div dir="{direction}" class="emergency"><div class="ai-title">🚨 {esc(T["emergency"])}</div><div style="font-size:12px;line-height:1.6">{esc(T["emergency_text"])}</div></div>',unsafe_allow_html=True)
        st.markdown(f'<div style="margin-top:10px;color:var(--muted);font-size:11px;font-weight:700">{esc(T["sources"])}</div>',unsafe_allow_html=True)
        for src in GUIDANCE.get(prediction,GUIDANCE["Normal"])["sources"]:
            st.markdown(f'<a href="{SOURCE_URLS[src]}" target="_blank" style="font-size:11px;color:#38B8D0;text-decoration:none">↗ {esc(src)}</a>',unsafe_allow_html=True)
