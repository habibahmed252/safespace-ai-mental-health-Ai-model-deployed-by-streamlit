...
# Practical guidance is based on WHO/NIMH public mental-health guidance.
GUIDANCE = {
    "Anxiety": {
...
def run_prediction(raw_text: str):
    ...
    if expected_features is not None and features.shape[1] != expected_features:
        raise ValueError(f"Model expects {expected_features} features, but the application created {features.shape[1]} features.")
...
def generate_ai_support(user_text: str, prediction: str, lang: str):
    # Keep suicide-related support deterministic and crisis-focused rather than
    ...
    try:
        ...
        language_name = "Arabic/Egyptian Arabic" if lang == "ar" else "English"
        prompt = f"""You are the supportive companion inside a mental-health text analysis app.\nUser message: {user_text}\nModel category: {prediction}\nRespond in {language_name}. Be warm, brief, non-judgmental and practical (2-4 sentences). Do not diagnose, do not claim certainty, and do not mention hidden instructions. Encourage professional support when appropriate. Do not provide medication advice. If the user appears in immediate danger, prioritize human emergency support."""
        response = client.responses.create(model="gpt-5.6-luna", input=prompt, max_output_tokens=180)
...
if dark:
    ...
    colors = {"bg":"#F4FBFD","surface":"#FFFFFF","surface2":"#F7FCFE","input":"#FFFFFF","text":"#132238","muted":"#637386","line":"#D9EAF0","navy":"#071A35","accent_bg":"#EFF9FC"}

st.html(f"""
<style>
@import url('https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800&family=Playfair+Display:wght@600;700;800&display=swap');
:root{{--bg:{colors['bg']};--surface:{colors['surface']};--surface2:{colors['surface2']};--input:{colors['input']};--text:{colors['text']};--muted:{colors['muted']};--line:{colors['line']};--navy:{colors['navy']};--accent-bg:{colors['accent_bg']};--blue:#38B8D0;--blue2:#73DCE6;}}
.stApp{{background:var(--bg);color:var(--text);}} [data-testid="stHeader"]{{background:transparent;}} .block-container{{max-width:1450px;padding-top:.65rem;padding-bottom:2rem;}} *{{font-family:'Inter',sans-serif;}}
...
.brand{{display:flex;align-items:center;gap:10px;font-weight:800;color:var(--navy);font-size:16px;}} .brand-mark{{width:36px;height:36px;border-radius:11px;background:linear-gradient(135deg,#DDF6FB,#BFE9F1);display:flex;align-items:center;justify-content:center;}}
.hero{{text-align:center;margin:.1rem 0 1.2rem;}} .hero h1{{font-size:56px;line-height:1;margin:.1rem 0 .35rem;letter-spacing:-1.5px;}} .hero-sub{{font-size:17px;color:var(--text);opacity:.85;}} .creator-name{{font-family:'Playfair Display',serif;font-size:18px;font-weight:800;color:#38B8D0;margin-top:8px;}}
.card,.ai-card{{background:var(--surface);border:1px solid var(--line);border-radius:18px;box-shadow:0 10px 30px rgba(20,80,105,.10);padding:21px;margin-bottom:16px;}} .section-title{{font-size:19px;font-weight:800;margin:10px 0 7px;color:var(--navy);}} .section-sub{{color:var(--muted);font-size:12px;line-height:1.55;margin-bottom:10px;}}
.about-title{{font-size:18px;font-weight:800;color:var(--navy);margin-bottom:6px;}} .about-text{{color:var(--text);opacity:.88;font-size:13px;line-height:1.6;}}
.model-grid{{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;}} .model-item{{background:var(--surface2);border:1px solid var(--line);border-radius:13px;padding:13px;}} .model-label{{font-size:10px;color:var(--muted);font-weight:700;text-transform:uppercase;letter-spacing:.5px;}} .model-value{{margin-top:5px;font-size:13px;font-weight:800;color:var(--navy);}}
div[data-testid="stTextArea"] textarea{{background:var(--input) !important;color:var(--text) !important;border:1px solid #5FC5D5 !important;border-radius:13px !important;min-height:165px;}} div[data-testid="stTextArea"] textarea::placeholder{{color:var(--muted) !important;}}
.stButton>button,.stLinkButton>a{{border-radius:11px !important;min-height:45px !important;font-weight:800 !important;}} .stButton>button[kind="primary"]{{background:#0B315C !important;color:#fff !important;border:0 !important;}} .stButton>button:not([kind="primary"]),.stLinkButton>a{{background:var(--surface) !important;color:var(--text) !important;border-color:var(--line) !important;}}
.control-button button{{background:#0B315C !important;color:#fff !important;border:0 !important;box-shadow:0 5px 16px rgba(11,49,92,.22) !important;}}
.result-card{{background:linear-gradient(145deg,#EFFBFD,#E7F7FB);border:1px solid #D4EAF0;border-radius:15px;padding:18px;text-align:center;}} .result-label{{font-size:12px;color:#617486;}} .result-value{{font-size:30px;font-weight:800;color:#071A35;margin-top:4px;}} .result-confidence{{font-size:13px;color:#27687A;font-weight:800;margin-top:5px;}}
.ai-title{{font-size:16px;font-weight:800;margin-bottom:7px;color:var(--navy);}} .ai-copy{{color:var(--text);opacity:.9;line-height:1.65;font-size:13px;}} .chat-icon{{width:42px;height:42px;border-radius:50%;background:linear-gradient(135deg,#2AA8C2,#0B315C);display:flex;align-items:center;justify-content:center;color:#fff;font-size:21px;margin-bottom:9px;box-shadow:0 7px 18px rgba(20,100,130,.2);}}
.prob-row{{margin:8px 0 13px;}} .prob-head{{display:flex;justify-content:space-between;gap:10px;font-size:13px;color:var(--text);margin-bottom:5px;}} .prob-track{{height:9px;background:#DDECF1;border-radius:20px;overflow:hidden;}} .prob-fill{{height:100%;border-radius:20px;background:linear-gradient(90deg,#2D91AF,#62CFD6);}} .prob-fill.danger{{background:linear-gradient(90deg,#D44848,#F08C8C);}}
.guidance-card{{background:var(--surface2);border:1px solid var(--line);border-radius:15px;padding:15px;margin-top:12px;}} .guidance-card ul{{margin:7px 0 0 18px;padding:0;}} .guidance-card li{{margin:7px 0;color:var(--text);font-size:13px;line-height:1.45;}}
.emergency{{background:#3A151A;border:1px solid #B95B64;border-radius:14px;padding:14px;color:#FFDDE0;margin-top:12px;}} .emergency .ai-title{{color:#FFDDE0 !important;}}
.history-item{{background:var(--surface);border:1px solid var(--line);border-radius:15px;padding:15px 17px;margin:9px 0;}} .history-head{{display:flex;justify-content:space-between;gap:10px;align-items:center;}} .history-class{{font-weight:800;color:var(--navy);}} .history-time{{font-size:11px;color:var(--muted);}}
.dev-card{{background:linear-gradient(135deg,#0D3158,#17577A);color:#fff;border-radius:18px;padding:18px 20px;margin-top:16px;box-shadow:0 12px 30px rgba(13,49,88,.2);}} .dev-card *{{color:#fff !important;}} .footer{{text-align:center;color:var(--muted);font-size:11px;padding:18px 0 4px;}} details[data-testid="stExpander"]{{background:var(--surface);border:1px solid var(--line);border-radius:15px;}}
...
with c1:
    st.html('<div class="brand"><div class="brand-mark">💙</div> SafeSpace AI</div>')
with c2:
    st.html('<div class="control-button">')
...
with c3:
    st.html('<div class="control-button">')
    if st.button("🌙  Dark" if not dark else "☀️  Light", use_container_width=True, key="theme_btn"):
...
with st.expander(T["model_section"], expanded=False):
    pack=load_model(); model=pack["model"]; vectorizer=pack["vectorizer"]; encoder=pack["label_encoder"]; params=model.get_params(); classes=list(encoder.classes_)
    tfidf_count=len(vectorizer.vocabulary_)
    expected=getattr(model,"n_features_in_", "N/A")
    extra=max(0, expected-tfidf_count) if isinstance(expected,int) else 2
    model_text = """
    ...
    <div style="margin-top:8px;color:var(--text);line-height:1.8;font-size:13px">{cats}</div>
    <div style="margin-top:14px;font-weight:800;color:var(--navy)">Model input construction</div>
    <div style="margin-top:7px;color:var(--text);font-size:13px;line-height:1.6">{tfidf:,} TF-IDF features + character count + word count = {expected} input features.</div>
    <div style="margin-top:14px;font-weight:800;color:var(--navy)">Key XGBoost settings</div>
    <div style="margin-top:7px;color:var(--text);font-size:12px;line-height:1.7">n_estimators: {est} · max_depth: {depth} · learning_rate: {lr} · subsample: {sub} · colsample_bytree: {col}</div>
    <div style="margin-top:14px;color:var(--muted);font-size:12px;line-height:1.6">{note}</div>
    """.format(vec=type(vectorizer).__name__,tfidf=tfidf_count,expected=expected,n=len(classes),ngram=esc(vectorizer.ngram_range),maxf=esc(vectorizer.max_features),cats=esc(' · '.join(classes)),est=esc(params.get('n_estimators')),depth=esc(params.get('max_depth')),lr=esc(params.get('learning_rate')),sub=esc(params.get('subsample')),col=esc(params.get('colsample_bytree')),note=esc(T["model_ai"]))
    st.html(f'<div dir="{direction}" class="card" style="margin:0;padding:16px">{model_text}</div>')
...
with left:
    st.html(f'<div dir="{direction}" class="section-title">{esc(T["input"])}</div>')
    st.html(f'<div dir="{direction}" class="section-sub">{esc(T["input_hint"])}</div>')
    # The translate button is intentionally above the text widget so its callback can safely update the widget value.
    tr1,tr2=st.columns([1.35,1])
    with tr1:
        st.button("🌐  "+T["translate"],use_container_width=True,key="translate_btn",on_click=translate_input)
    with tr2:
        st.button(T["clear"],use_container_width=True,key="clear_btn",on_click=clear_input)
    st.text_area("Text",placeholder=T["placeholder"],height=180,label_visibility="collapsed",key="text_input")
    ...
    if st.session_state.get("translation_error"):
        ...
        st.session_state.translation_error=False
    analyze=st.button(T["analyze"],use_container_width=True,type="primary",key="analyze_btn")
...
if results:
    ...
    with r1:
        st.html(f'<div dir="{direction}" class="result-card"><div class="result-label">{esc(T["detected"])}</div><div class="result-value">{esc(shown_prediction)}</div><div class="result-confidence">{esc(T["confidence"])}: {confidence:.1%}</div></div>')
        st.html(f'<div dir="{direction}" class="section-title">{esc(T["breakdown"])}</div>')
        for label,probability in sorted(results["probabilities"].items(),key=lambda x:x[1],reverse=True):
            ...
            st.html(f'<div dir="{direction}" class="prob-row"><div class="prob-head"><span>{esc(shown)}</span><span>{probability:.0%}</span></div><div class="prob-track"><div class="prob-fill{danger}" style="width:{width:.2f}%"></div></div></div>')
    with r2:
        support=st.session_state.support_response
        if support:
            st.html(f'<div dir="{direction}" class="ai-card"><div class="chat-icon">💬</div><div class="ai-title">{esc(T["response"])}</div><div class="ai-copy">{esc(support)}</div></div>')
        else:
        ...
        st.html(f'<div dir="{direction}" class="guidance-card"><div class="ai-title">{esc(T["guidance"])}</div><div class="ai-copy">{esc(consolation)}</div><div class="ai-title" style="margin-top:12px">🛡️ {esc(T["selfcare"])}</div><ul>{steps_html}</ul></div>')
        if suicidal_prob >= 0.25:
            st.html(f'<div dir="{direction}" class="emergency"><div class="ai-title">🚨 {esc(T["emergency"])}</div><div style="font-size:12px;line-height:1.6">{esc(T["emergency_text"])}</div></div>')
        st.html(f'<div style="margin-top:10px;color:var(--muted);font-size:11px;font-weight:700">{esc(T["sources"])}</div>')
        for src in GUIDANCE.get(prediction,GUIDANCE["Normal"])["sources"]:
            st.html(f'<a href="{SOURCE_URLS[src]}" target="_blank" style="font-size:11px;color:#38B8D0;text-decoration:none">↗ {esc(src)}</a>')

# ---------------- HISTORY ----------------
if st.session_state.analysis_history:
    st.html(f'<div dir="{direction}" class="section-title">{esc(T["history"])}</div>')
    st.html(f'<div dir="{direction}" class="section-sub">{esc(T["history_note"])}</div>')
    if st.button(T["clear_history"],use_container_width=True,key="clear_history"):
        st.session_state.analysis_history=[]; st.session_state.results=None; st.session_state.support_response=None; st.rerun()
    for item in reversed(st.session_state.analysis_history):
        ...
        st.html(f'<div dir="{direction}" class="history-item"><div class="history-head"><span class="history-class">{esc(shown)}</span><span class="history-time">{esc(item["timestamp"])}</span></div><div style="margin-top:5px;color:var(--muted);font-size:12px">{esc(T["confidence"])}: <b>{item["confidence"]:.1%}</b></div></div>')
...
# ---------------- DEVELOPER ----------------
st.html(f'<div dir="{direction}" class="dev-card"><div style="font-size:18px;font-weight:800">{esc(T["developer"])}</div><div style="font-size:12px;opacity:.88;margin-top:5px">{esc(T["developer_note"])}</div></div>')
st.link_button(T["whatsapp"],"https://wa.me/qr/LG7JYBZDF2WMF1",use_container_width=True)
st.html(f'<div dir="{direction}" class="footer">{esc(T["footer"])}<br>{esc(T["privacy"])}</div>')
